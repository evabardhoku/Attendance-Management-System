    ...
    public partial class OraMesimore : UserControl
    {
        ...
        private void btnHapKlasen_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                btnOpenClassroom.BackColor = System.Drawing.Color.SteelBlue;
                btnOpenClassroom.ForeColor = Color.White;
                tcpListener.Stop();
            }
        }
        ...
        private bool IsStudentInSelectedGroup(string cardNumber)
        {
            ...
            SELECT COUNT(*) 
            FROM StudentCard sc
            JOIN Schedule s ON sc.LearningGroupID = s.LearningGroupID
            JOIN Users u ON sc.StudentID = u.UserID
            WHERE sc.CardNumber = @CardNumber
              AND s.LearningGroupID = @GroupID
              AND s.SubjectID = @SubjectID
              AND s.InstructorID = @InstructorID
              AND s.DayOfWeek = @CurrentDay
